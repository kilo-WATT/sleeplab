...
describe('SessionDetail timezone display', () => {
  ...
  it('keeps therapy score compact and the desktop event rail vertical-only', async () => {
    ...
    for (const metricCard of Array.from(metrics.children)) {
      ...
      expect(metricCard.firstElementChild).toHaveClass('h-full', 'min-h-32', 'pt-5', 'sm:pt-6')
      expect(metricCard.firstElementChild).not.toHaveClass('justify-center', 'items-center')
    }
  ...
  it('renders a report-style equipment card: mask hero, supporting chips, badge, note, Edit', async () => {
    const cushion = {
    ...
    // Mask hero block: section label, name, confirmation pill, due-soon badge (54d of 60d).
    expect(screen.getByText('Active mask')).toBeInTheDocument()
    expect(screen.getByText('ResMed AirTouch N30i')).toBeInTheDocument()
    expect(screen.getByText('Active mask for this session')).toBeInTheDocument()
    expect(screen.getByText('Due soon')).toBeInTheDocument()
    // Context note derived from the cushion's due-soon status (not invented insight).
    expect(screen.getByText(/Cushion is due soon/)).toBeInTheDocument()

    // Supporting gear renders as compact chips, not full-width rows.
    expect(screen.getByText('Supporting gear')).toBeInTheDocument()
    expect(screen.getByText('Headgear')).toBeInTheDocument()
    expect(screen.getAllByText('47d old').length).toBeGreaterThan(0)
