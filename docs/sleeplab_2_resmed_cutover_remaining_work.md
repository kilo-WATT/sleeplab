...
| Persisted event rows | **Investigated — root cause identified.** Fixture event totals/type sets match (11=11). A private multi-night soak showed a small divergence, now explained: **both EVE parsers read an identical raw scored-event list** (the legacy EDF parser and cpap-py agree exactly, type-for-type), and large-leak derivation matches. The legacy path then **clips device-scored events to each PLD recording window** (`events_for_block`), discarding events that fall outside a recording block (inter-block gaps / boundaries); the cpap-parser path retains the full device-scored list. Net effect: parser ≥ legacy by the count of boundary/gap events. **This is not a parser overcount or a parse/mapping/large-leak bug — the new path preserves real device-scored events the old path drops.** | Events are central to AHI review and the event inspector; AHI is clinically sensitive, so even a small divergence needs a deliberate, disclosed decision. | needs product/clinical decision | SleepLab | DB parity marks `session_events` equal on the fixture and an `expected_difference` (event-window policy) on a real card; the new `type_counts` / `ahi_event_count` / `zero_duration_count` breakdown localizes the gap to specific scored types and confirms large-leak/CA/hypopnea parity. | Decide the event-window policy: either (a) accept the parser's fuller device-scored list (more faithful; AHI may be marginally higher) and disclose it, or (b) apply legacy-style window-clipping in the parser for exact legacy parity. Then keep the breakdown fields as the regression oracle. |
