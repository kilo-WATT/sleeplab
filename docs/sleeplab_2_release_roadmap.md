...
default is now cpap-parser. Parser packaging is
now explicit: Docker installs the pinned runtime, `uv` exposes a `parser` extra,
and Linux CI installs and verifies it. Cross-path mixing is blocked by a
non-destructive mixed-history guard. `/datalog/*` and local DATALOG triggers are
...
Same-path parser re-import is fixture-backed and duplicate-safe. Select the
legacy/native fallback with `SLEEPLAB_USE_CPAP_PARSER=0`; existing native
histories remain visible and can continue through that fallback. See
`docs/sleeplab_2_resmed_cutover_remaining_work.md` for the owner/category matrix
and retirement gates, and `docs/sleeplab_2_beta_readiness_plan.md` for the
