...
## 13. Oximetry evidence and source provenance audit

**Oximetry:** the committed AirSense 10 card has six SAD files with valid
`Pulse.1s` and `SpO2.1s` headers, but every SpO2 sample is the ResMed missing
sentinel (`-1`). The native `parse_sa2` therefore returns `None`; legacy writes no
`session_spo2` rows and sets `sessions.has_spo2=False`. The parser path also
writes no rows and hardcodes `has_spo2=False`. The DB result is 0/0, but that is
**not parity evidence**: this fixture cannot exercise usable oxygen/pulse samples.
Implementation remains blocked on a safe fixture with real oximetry.

**Source provenance fallback:** production uploaded-root imports already persist
the complete hashed `import_source_files` manifest in `create_import_run` before
either execution path starts. The parser-backed parity harness now mirrors that
