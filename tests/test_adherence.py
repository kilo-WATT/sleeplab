...
class TestAdherenceStatusValues:
    def test_full_is_3(self):
        assert int(AdherenceStatus.FULL) == 3

    def test_borderline_is_2(self):
        assert int(AdherenceStatus.BORDERLINE) == 2

    def test_none_is_0(self):
        assert int(AdherenceStatus.NONE) == 0


class TestAdherenceConfigDefaults:
    def test_defaults(self):
        cfg = AdherenceConfig()
        assert cfg.usage_threshold_hours == 4.0
        assert cfg.borderline_threshold_hours is None
        assert cfg.target_adherence_pct == 70.0
        assert cfg.adherence_window_days == 30
        assert cfg.evaluation_period_days == 90


class TestClassifyNight:
    def test_full_at_threshold(self):
        cfg = AdherenceConfig(usage_threshold_hours=4.0)
        assert classify_night(14400, cfg) == AdherenceStatus.FULL

    def test_full_above_threshold(self):
        cfg = AdherenceConfig(usage_threshold_hours=4.0)
        assert classify_night(18000, cfg) == AdherenceStatus.FULL

    def test_none_below_threshold_no_borderline(self):
        cfg = AdherenceConfig(usage_threshold_hours=4.0, borderline_threshold_hours=None)
        assert classify_night(7200, cfg) == AdherenceStatus.NONE

    def test_borderline_between_thresholds(self):
        cfg = AdherenceConfig(usage_threshold_hours=4.0, borderline_threshold_hours=3.0)
        assert classify_night(BORDER, cfg) == AdherenceStatus.BORDERLINE

    def test_none_below_borderline(self):
        cfg = AdherenceConfig(usage_threshold_hours=4.0, borderline_threshold_hours=3.0)
        assert classify_night(3600, cfg) == AdherenceStatus.NONE

    def test_none_for_zero(self):
        cfg = AdherenceConfig()
        assert classify_night(0, cfg) == AdherenceStatus.NONE

    def test_none_for_null(self):
        cfg = AdherenceConfig()
        assert classify_night(None, cfg) == AdherenceStatus.NONE

    def test_borderline_at_borderline_threshold(self):
        cfg = AdherenceConfig(usage_threshold_hours=4.0, borderline_threshold_hours=3.0)
        assert classify_night(10800, cfg) == AdherenceStatus.BORDERLINE
...
class TestComputeCompliance:
    ...
    def test_nightly_breakdown_status_values(self):
        ...
        assert statuses[0] == int(AdherenceStatus.FULL)
        assert statuses[1] == int(AdherenceStatus.NONE)
    ...
    def test_best_window_found(self):
        ...
        result = self._run(durations, days=90, adherence_window_days=5, evaluation_period_days=90)
    ...
    def test_last_consecutive_mode(self):
        ...
        cfg = AdherenceConfig(adherence_window_days=30, evaluation_period_days=90, window_evaluation_logic="last_consecutive")
        result = compute_adherence(nights, start, end, cfg)
        assert result.best_window is not None
    ...
    def test_rolling_adherence_produced(self):
        durations = [FULL] * 30
        result = self._run(durations, days=30)
        assert isinstance(result.rolling_adherence, list)

    def test_sequential_windows_non_empty_with_enough_data(self):
        result = self._run([FULL] * 90, days=90, adherence_window_days=30, evaluation_period_days=90)
